...
export function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="text-center py-10 px-6">
      <p className="font-display text-lg text-[var(--color-ink)]">Something went wrong</p>
      <p className="text-sm text-black/45 mt-1">{message}</p>
      {onRetry && (
        <button onClick={onRetry} className="mt-3 text-sm font-semibold bond-accent">
          Try again
        </button>
      )}
    </div>
...
export function Skeleton({ className }: { className?: string }) {
  return <div className={clsx('animate-pulse rounded-xl bg-black/[0.06]', className)} />;
}
