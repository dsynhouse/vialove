...
import { useAddEvent, useEvents, useRemoveEvent } from '../lib/queries';
import { Card, SectionHeading, Pill, Button, EmptyState, Skeleton } from '../components/ui';
...
export default function PlanCalendar() {
  const { activeBond } = useBond();
  const { data: events = [], isLoading } = useEvents(activeBond?.id);
  const addEvent = useAddEvent(activeBond?.id);
  const removeEvent = useRemoveEvent(activeBond?.id);
    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <div>
        ...
        <Card className="p-6">
          ...
          <Button onClick={handleAdd} disabled={!title.trim() || addEvent.isPending} className="w-full justify-center">
      ...
      <div>
          ...
          <div className="space-y-2">
            {upcoming.map((e) => (
              ...
              >
                ...
                <button onClick={() => removeEvent.mutate(e.id)} className="text-black/25 hover:text-black/50 shrink-0">
                  <Trash2 size={15} />
                </button>
              </motion.div>
            ))}
        ...
        {past.length > 0 && <p className="text-xs text-black/30 mt-3">{past.length} past event{past.length === 1 ? '' : 's'}</p>}
      </div>
