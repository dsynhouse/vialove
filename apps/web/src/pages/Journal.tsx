...
import { useAddJournalEntry, useJournal } from '../lib/queries';
...
export default function Journal() {
  ...
  const { data: entries = [], isLoading } = useJournal(activeBond?.id);
  const addEntry = useAddJournalEntry(activeBond?.id);
  ...
  const filtered = entries.filter((e) => e.kind === kind).sort((a, b) => b.createdAt.localeCompare(a.createdAt));

  function handleSubmit() {
    if (!content.trim()) return;
    addEntry.mutate(
      { kind, prompt, content: content.trim(), visibility },
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card className="p-6 paper-texture relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mt-4 flex-wrap gap-3">
          ...
          <Button onClick={handleSubmit} disabled={!content.trim() || addEntry.isPending}>
            {addEntry.isPending ? 'Adding…' : 'Add to journal'}
          </Button>
        </div>
      </Card>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <AnimatePresence>
              {filtered.map((e) => {
                ...
                return (
                  <motion.div key={e.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                    <Card className="p-5 h-full flex flex-col paper-texture">
                      <p className="text-[11px] font-semibold uppercase tracking-wide bond-accent mb-1.5">{e.prompt}</p>
                      <p className={e.kind === 'fun' ? 'font-hand text-xl leading-snug flex-1' : 'text-[15px] leading-relaxed flex-1'}>
                        {e.content}
                      </p>
                      <div className="flex items-center justify-between mt-4 pt-3 border-t border-black/5">
                        ...
                        {e.visibility === 'private' ? <Lock size={13} className="text-black/30" /> : <Users size={13} className="bond-accent" />}
                      </div>
                    </Card>
                  </motion.div>
                );
