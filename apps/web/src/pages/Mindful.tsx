...
import { useLogMindfulSession, useMindfulLogs } from '../lib/queries';
import { Card, SectionHeading, Pill, EmptyState, Skeleton } from '../components/ui';
...
export default function Mindful() {
  ...
  const { data: logs = [], isLoading } = useMindfulLogs(activeBond?.id);
  const logSession = useLogMindfulSession(activeBond?.id);
  ...
  const completedToday = logs.filter((l) => l.completedAt.slice(0, 10) === new Date().toISOString().slice(0, 10));
  ...
  return (
    <Card className="p-8 text-center">
      ...
      <div className="flex items-center justify-center gap-3 mt-6">
        <button onClick={onClose} className="px-4 py-2.5 rounded-full text-sm font-semibold text-black/50 hover:bg-black/5">
          Back
        </button>
        {!done ? (
          <button
