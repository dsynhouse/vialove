...
import { useSendSupportSignal } from '../lib/queries';
...
export default function Village() {
  const { activeBond, otherPerson } = useBond();
  const sendSignal = useSendSupportSignal(activeBond?.id);
  const [sent, setSent] = useState(false);

  if (!activeBond) return null;

  function handleSend() {
    sendSignal.mutate(undefined, {
      onSuccess: () => {
        setSent(true);
    ...
    <div className="space-y-8 max-w-2xl">
      ...
      <Card className="p-6 bond-bg-soft border-none flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-[13px] text-black/50">
            {otherPerson ? `Quietly let ${otherPerson.name} know, without needing to explain yet.` : 'Invite your partner to receive this signal.'}
          </p>
        </div>
        <Button onClick={handleSend} size="sm" disabled={!otherPerson || sendSignal.isPending}>
      ...
      <AnimatePresence>
        {sent && (
          ...
          >
            {otherPerson?.name} will see this instantly.
          </motion.p>
        )}
      </AnimatePresence>
