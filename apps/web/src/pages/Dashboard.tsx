...
import { useCheckIns, useEvents, useJournal } from '../lib/queries';
...
export default function Dashboard() {
  ...
  const { data: checkIns = [], isLoading: checkInsLoading } = useCheckIns(activeBond?.id);
  const { data: events = [] } = useEvents(activeBond?.id);
  const { data: journal = [] } = useJournal(activeBond?.id);

  if (!activeBond || !activePerson || !user) return null;

  const today = todayStr();
  const myCheckIn = checkIns.find((c) => c.personId === activePerson.id && c.date === today);
  const theirCheckIn = otherPerson && checkIns.find((c) => c.personId === otherPerson.id && c.date === today);
  const myStreak = checkInStreak(checkIns, activePerson.id);

  const upcoming = events
    .filter((e) => `${e.date}T${e.time}` >= new Date().toISOString().slice(0, 16))
  ...
  const sharedJournalCount = journal.filter((j) => j.visibility === 'shared').length;
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <h1 className="font-display text-3xl sm:text-4xl text-[var(--color-ink)]">Hi {activePerson.name}</h1>
        ...
        <Card className="p-6">
          ...
          {checkInsLoading ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <PersonMoodRow
                name={otherPerson?.name ?? 'Partner'}
                mood={theirCheckIn?.visibility === 'shared' ? theirCheckIn.mood : undefined}
                done={!!theirCheckIn}
                muted={!theirCheckIn || theirCheckIn.visibility !== 'shared'}
                pending={!otherPerson}
              />
            </div>
          )}
...
function PersonMoodRow({
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 flex-1">
      <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl ${done ? 'bond-bg-soft' : 'bg-black/[0.04]'}`}>
        {mood ? <MoodBadge mood={mood} /> : done ? '🔒' : '—'}
      </div>
      <span className="text-xs font-medium text-black/50 truncate max-w-[80px]">{name}</span>
      {pending && <span className="text-[10px] text-black/30">pending invite</span>}
      {!pending && muted && done && <span className="text-[10px] text-black/30">private</span>}
    </div>
  );
}
